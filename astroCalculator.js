globalThis.AstroCalculator = {
  calcNatal: (date, time, Astro) => {
    const jd = Astro.toJDt(date, time);
    const T = (jd - 2451545) / 36525;
    const natalLons = {};
    globalThis.AstroCfg.PLANETS.forEach(p => { natalLons[p] = Astro.getLon(p, T); });
    return natalLons;
  },
  ...
  calcRawScores: (sJD, eJD, pairData, aspEn, orb, Astro) => {
    const step = Math.max(1, Math.round((eJD - sJD) / 1000));
    const active = pairData.filter(p => p.vis);
    const asps = globalThis.AstroCfg.ASPECTS.filter(a => aspEn[a.angle]);
    const scores = [];
    for (let jd = sJD; jd <= eJD; jd += step) {
      const T = (jd - 2451545) / 36525;
      let tot = 0;
